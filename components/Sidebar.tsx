...
import { LayoutDashboard, Kanban, Users, CreditCard, Settings, LogOut, Zap, Moon, Sun, CheckSquare, UserCog, History, ListChecks, X } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onChangeView, isDark, toggleTheme, user, onLogout, isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside className={`
        fixed left-0 top-0 h-full glass-panel border-r border-white/10 dark:border-white/10 flex flex-col z-50 transition-transform duration-300 ease-in-out
      ...
      `}>
        ...
        <div className="p-6 flex items-center gap-3 mb-2">
          ...
          <span className="text-lg font-thin tracking-wide block md:hidden lg:block text-glow text-slate-800 dark:text-white">GlassFlow</span>
        </div>
        ...
        <nav className="flex-1 px-3 space-y-1 overflow-y-auto custom-scrollbar">
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                <item.icon size={20} className={`transition-colors shrink-0 ${isActive ? 'text-indigo-600 dark:text-indigo-400' : 'group-hover:text-indigo-500 dark:group-hover:text-indigo-300'}`} />
                <span className={`text-sm font-medium block md:hidden lg:block ${isActive ? 'opacity-100' : 'opacity-80'}`}>{item.label}</span>
                {isActive && (
                  <div className="ml-auto w-1.5 h-1.5 rounded-full bg-indigo-500 dark:bg-indigo-400 shadow-[0_0_8px_rgba(129,140,248,0.8)] block md:hidden lg:block" />
                )}
              </button>
            );
          })}
        ...
        <div className="p-4 mt-auto space-y-2">
          {user && (
            <div className="flex items-center gap-3 px-3 py-2 mb-2 rounded-xl bg-white/50 dark:bg-white/5 border border-slate-100 dark:border-white/5">
              <img src={user.avatarUrl} alt={user.name} className="w-8 h-8 rounded-full bg-indigo-100 dark:bg-indigo-900 shrink-0" />
          ...
          >
            ...
            <span className="text-sm font-medium block md:hidden lg:block">{isDark ? 'Light Mode' : 'Dark Mode'}</span>
          </button>
          ...
          >
            <LogOut size={20} />
            <span className="text-sm font-medium block md:hidden lg:block">Sign Out</span>
          </button>
        </div>
      </aside>
    </>
